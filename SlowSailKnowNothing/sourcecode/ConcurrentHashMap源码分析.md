...
final V putVal(K key, V value, boolean onlyIfAbsent) {
    ...
    for (Node<K,V>[] tab = table;;) {
        ...
        //如果hash计算得到的桶位置元素的hash值为MOVED，证明正在扩容，那么协助扩容
            ...
            synchronized (f) {
                if (tabAt(tab, i) == f) {// 防止有一个元素做完操作之后将f变成了树
                    //普通链表节点
                    if (fh >= 0) {//判断hash值大于0说明是链表
                        binCount = 1;
...
集合里面维护长度的方式。不同的线程，可能会同时对数组的长度进行修改。将长度不同的线程维护一个对应的val，可以用一个数组来表示。然后如果线程对数组中的其中一个val无法做修改，就修改另外一个val，最后集合的长度就是val和basecount的加和：

![image-20211028191549787](https://gitee.com/SlowSail/blogIMg/raw/master//img/image-20211028191549787.png)
...
![image-20211028193537383](https://gitee.com/SlowSail/blogIMg/raw/master//img/image-20211028193537383.png)
